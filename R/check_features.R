...
getUISettingsOld <- function()
{
    path <- getUISettingsPathOld()
    if (!file.exists(path))
    {
        ret <- getDefaultUISettingsOld()
        saveUISettingsOld(ret)
    }
    else
...
getDefaultUISettingsOld <- function()
{
...
saveUISettingsOld <- function(settings)
{
...
getCheckFeaturesSessionPathOld <- function(session)
{
...
#' @export
importCheckFeaturesSessionOld <- function(sessionIn, sessionOut, fGroups, overWrite = FALSE)
{
    # UNDONE: docs
...
setMethod("checkFeatures", "featureGroups", function(fGroups, session, rtWindow, clearSession)
{
    ...
    server <- function(input, output, session)
    {
        ...
        observeEvent(input$resetSettings, {
            doApplySettings(getDefaultUISettingsOld())
            syncInputSettings()
            shinyjs::disable("saveApplySettings")
        })
