...
setMethod("export", "featureGroups", function(obj, type, out, IMS = FALSE)
{
    ...
    {
        if (!isFALSE(IMS))
            warning("Exporting IMS features is not supported; mobility data will not be exported.", call. = FALSE)
                                                              ...
                                                              EICParams = getDefEICParams(window = 0), parallel = TRUE)
{
    ...
    gNames <- names(obj)
    gCount <- length(obj)
    EICs <- getFeatureEIXs(obj, type = "EIC", EIXParams = getDefEICParams(window = 0))
...
#'   data.
#'
#'   In IMS workflows with post mobility assignment (see \link[=assignMobilities_feat]{assignMobilities}), any IMS
#'   features are excluded for the assignment of internal standards (\code{featNorm="istd"}) or calculation of TICs
#'   (\code{featNorm="tic"}). Furthermore, the normalized intensities and areas for IMS features are copied from
                                                ...
                                                ISTDMZWindow, minISTDs, ...)
{
    ...
    else if (featNorm == "tic")
    {
        fGroupsInt <- if (hasIMS(fGroups))
            selectIMSFilter(fGroups, "maybe", verbose = FALSE) # exclude IMS features
        else
            fGroups
    ...
    # HACK: for IMS features we keep normalization equal to their precursors
    if (hasIMS(fGroups))
    {
        updatedFeatures <- lapply(updatedFeatures, function(ft)
        {
...
#' Assign Ion Mobility and \acronym{CCS} values to features
#'
...
#' @param chromPeakParams A \code{list} with peak detection parameters for the re-integration of IMS features,
...
#'   (for IMS feature re-integration), generated by \code{\link{getDefEIMParams}} and
#'   \code{\link{getDefEICParams}}.
#' @param peakRTWindow The retention time tolerance (in seconds) for detected peaks to be used for re-integration (Step
#'   3, see \verb{Mobility assignment}). This should be kept low to ensure no closely eluting features from other
#'   compounds are used. Note that peaks with retention times outside the RT window of the IMS precursor are always
#'   ignored.
#' @param fallbackEIC Set to \code{TRUE} to allow updating intensities and areas from raw EICs in Step 3 (see
#'   \verb{Mobility assignment}).
#' @param calcArea controls how the area is calculated when updating from raw EIC data (see \code{fallbackEIC}):
...
#'
#'   \item The detected mobility peaks in an EIM are then used to form \emph{IMS features}. These features inherit
#'   their LC-MS properties (RT, \emph{m/z}, etc) from the corresponding \emph{IMS precursor}, \emph{i.e.} the feature for
#'   which the EIM was created. The mobility peak centroids and ranges are used to assign IMS data to the IMS
#'   features. Multiple mobility peaks within the same EIM result in multiple IMS features, and each are linked to
#'   the same IMS precursor. The linkage is especially useful to keep a relation between \emph{e.g.} protomers.
#'
...
#'
#'   \item Any IMS features that could not be re-integrated (either by peak detection or EIC fallback) are removed.
#'
#'   \item The feature grouping is updated: the IMS features with close mobilities (defined by \code{mobWindow})
#'   within a feature group are split-off into new feature groups and linked to the original \emph{IMS precursor}
#'   feature group. This is performed by the \link[=groupFeaturesGreedy]{greedy grouping algorithm}. LC-MS properties
#'   and most other data such as feature group qualities and scores (\code{\link{calculatePeakQualities}}), adduct
#'   annotations (\emph{e.g.} \code{\link{selectIons}}), predicted concentrations and toxicities
#'   (\code{\link{calculateConcs}} and \code{\link{calculateTox}}) and internal standards for intensity normalization
#'   (\code{\link{normInts}}) are copied from the IMS precursors to the IMS feature groups.
#'
#'   }
#'
#'   Note that re-running \code{assignMobilities} will first remove any existing IMS features.
#'
#' @section IMS features: The features (and feature groups) with IMS properties are referred to \emph{IMS features} (and
#'   \emph{IMS feature groups}). These are referred to \emph{orphans} if their link to the original IMS precursor is
#'   removed (in \emph{post} workflows, see previous section) or non-existent (\emph{direct} workflows). The formation
#'   of orphans in \emph{post} workflows typically occurs by removal of IMS precursors by subsetting or filtering
#'   operations.
#'
