...
# callback for executeMultiProcess()
processSiriusFormulasBatch <- function(cmd, exitStatus, retries)
{
    noResult <- forms <- data.table(neutral_formula = character(0), formula = character(0),
                                    adduct = character(0), score = numeric(0), MSMSScore = numeric(0),
                                    isoScore = numeric(0), byMSMS = logical(0),
    ...
    resFile <- if (cmd$isPre44) "summary_sirius.csv" else "formula_candidates.csv"
    
    ret <- mapply(cmd$msFNames, cmd$hashes, SIMPLIFY = FALSE, FUN = function(msFName, hash)
    {
        resultPath <- getSiriusResultPath(cmd$outPath, msFName, cmd$cmpName, cmd$isPre44)
        ...
        saveCacheData("formulasSirius", forms, hash, cmd$cacheDB)
        return(forms)
    })
    names(ret) <- cmd$gNames
...
generateFormulasSiriusBatch <- function(fGroups, MSPeakLists, relMzDev = 5, adduct = "[M+H]+", elements = "CHNOP",
                                        ...
                                        logPath = file.path("log", "sirius"), maxProcAmount = getOption("patRoon.maxProcAmount"))
{
    ...
    doSIRIUS <- function(featMZs, groupPeakLists)
    {
        ...
        fGroupBatches <- split(doFGroups, ceiling(seq_along(doFGroups) / batchSize))
        
        cmdQueue <- lapply(seq_along(fGroupBatches), function(batch)
        {
            grps <- fGroupBatches[[batch]]
            plmzs <- lapply(grps, function(g) groupPeakLists[[g]][["MS"]][precursor == TRUE, mz])
            mspls <- lapply(groupPeakLists[grps], "[[", "MS")
            msmspls <- lapply(groupPeakLists[grps], "[[", "MSMS")
            cmd <- getSiriusCommandBatch(plmzs, mspls, msmspls, profile, adduct, relMzDev, elements,
                                         database, noise, cores, FALSE, NULL, topMost, extraOpts, isPre44)
            logf <- if (!is.null(logPath)) file.path(logPath, paste0("sirius-form-batch_", batch, ".txt")) else NULL
            
            return(c(list(hashes = hashes[grps], adduct = adduct, cacheDB = cacheDB, logFile = logf,
                          gNames = grps), cmd))
        })
        ...
        if (length(cmdQueue) > 0)
        {
            ...
            ret <- executeMultiProcess(cmdQueue, processSiriusFormulasBatch, errorHandler = function(cmd, exitStatus, retries)
            {
                stop(sprintf("Fatal: Failed to execute SIRIUS for:\n%s\nExit code: %d - Log: %s",
                             paste0(cmd$gName, collapse = ", "), exitStatus, cmd$logFile))
            }, maxProcAmount = maxProcAmount)
            ret <- unlist(ret, recursive = FALSE) # flatten batches
