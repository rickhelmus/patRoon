                                   ...
                                   logPath = file.path("log", "sirius"), maxProcAmount = getOption("patRoon.maxProcAmount"))
{
    ...
    for (anai in seq_along(anaInfo$analysis))
    {
        ...
        cmdQueue <- sapply(gNames, function(grp)
        {
            ...
            return(c(list(hash = hash, adduct = adduct, cacheDB = cacheDB, stdoutFile = logf), cmd))
        }, simplify = FALSE)
        ...
        if (length(cmdQueue) > 0)
        {
            ...
            formTable[[ana]] <- executeMultiProcess(cmdQueue, processSiriusFormulas, maxProcAmount = maxProcAmount)
