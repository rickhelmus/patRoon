...
setMethod("initialize", "formulas", function(.Object, ...)
{
    ...
    .Object@scoreRanges <- sapply(.Object@formulas, calculateFormScoreRanges, mergedConsensusNames(.Object),
                                  simplify = FALSE)
                                                ...
                                                normalizeScores = "none", excludeNormScores = NULL)
{
    ...
    if (normalizeScores != "none")
    {
        fTable <- mapply(fTable, x@scoreRanges, SIMPLIFY = FALSE, FUN = normalizeFormScores,
                         MoreArgs = list(mConsNames = mergedConsensusNames(x), normalizeScores == "minmax",
                                             ...
                                             excludeNormScores = NULL, useGGPlot2 = FALSE)
{
    ...
    if (normalizeScores != "none")
        formTable <- normalizeFormScores(formTable, obj@scoreRanges[[groupName]], mergedConsensusNames(obj),
...
#### WIP
#### UNDONE: replace formulas with below when finished

#' @include feature_annotations.R
NULL

#' @export
formulasFA <- setClass("formulasFA", slots = c(featureFormulas = "list"),
                       contains = "featureAnnotations")

setMethod("initialize", "formulasFA", function(.Object, ...)
{
    .Object <- callNextMethod(.Object, ...)
...
#' @rdname formulas-class
formulasConsensusFA <- setClass("formulasConsensusFA",
                                slots = c(mergedConsensusNames = "character"),
                                contains = "formulasFA")
setMethod("mergedConsensusNames", "formulasConsensusFA", function(obj) obj@mergedConsensusNames)
...
#'
#' @export
setMethod("annotations", "formulasFA", function(obj, features = FALSE) if (features) obj@featureFormulas else callNextMethod())

#' @templateVar class formulas
#' @templateVar what analyses
#' @template strmethod
#' @export
setMethod("analyses", "formulasFA", function(obj) names(obj@featureFormulas))

#' @export
setMethod("defaultExclNormScores", "formulasFA", function(obj) character())

setMethod("annScoreNames", "formulasFA", function(obj) formulaScorings()$name)

#' @describeIn formulas Show summary information for this object.
#' @export
setMethod("show", "formulasFA", function(object)
{
    callNextMethod()
...
#'   formula table for the feature group specified by \code{j} is returned.
#' @export
setMethod("[[", c("formulasFA", "ANY", "ANY"), function(x, i, j)
{
    assertExtractArg(i)
    if (!missing(j))
        assertExtractArg(j)
...
#' @export
#'
setMethod("as.data.table", "formulasFA", function(x, fGroups = NULL, fragments = FALSE, countElements = NULL,
...
#'
#' @export
setMethod("filter", "formulasFA", function(obj, ..., OM = FALSE, negate = FALSE)
{
    ac <- checkmate::makeAssertCollection()
    aapply(checkmate::assertFlag, . ~ OM + negate, fixed = list(add = ac))
    checkmate::reportAssertions(ac)
    ...
    if (OM)
    {
        cat("Filtering OM formulas... ")
        oldn <- length(obj)
        
        obj@groupAnnotations <- pruneList(lapply(annotations(obj), function(annTable)
        {
                                              ...
                                              uniqueFrom = NULL, uniqueOuter = FALSE, rankWeights = 1, labels = NULL)
{
    ...
    ac <- checkmate::makeAssertCollection()
    checkmate::assertList(allFormulas, types = "formulasFA", min.len = 2, any.missing = FALSE,
                          unique = TRUE, .var.name = "...", add = ac)
    checkmate::assertNumber(rankWeights, lower = 0, finite = TRUE, add = ac)
    checkmate::assertCharacter(labels, min.chars = 1, len = length(allFormulas), null.ok = TRUE, add = ac)
    checkmate::reportAssertions(ac)
