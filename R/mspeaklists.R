...
#'   
#'   For \code{spectrumSimilarityIMS}: passed to \code{spectrumSimilarity}
#'
...
setMethod("averageMSPeakLists", "MSPeakLists", function(obj)
{
    ...
    else if (is.null(avgPLists))
    {
        doAvg <- function(mode, ...)
        {
            allPL <- sapply(gNames, function(grp)
            {
        ...
        allMSPLMSAvg <- doAvg("MS", obj@avgPeakListArgs$clusterMzWindow, obj@avgPeakListArgs$topMost,
                              ...
                              obj@avgPeakListArgs$maxRelCumIntensity, obj@avgPeakListArgs$method, TRUE,
        ...
        allMSPLMSMSAvg <- doAvg("MSMS", obj@avgPeakListArgs$clusterMzWindow, obj@avgPeakListArgs$topMost,
                                obj@avgPeakListArgs$minIntensityPre, obj@avgPeakListArgs$minIntensityPost,
                                obj@avgPeakListArgs$relMinAbundance, obj@avgPeakListArgs$absMinAbundance,
                                obj@avgPeakListArgs$maxRelCumIntensity, obj@avgPeakListArgs$method, TRUE, FALSE, FALSE,
...
#' @param maxRelCumIntensity The maximum relative cumulative intensity of a peak, calculated in descending order (most
#'   intense peaks first). For instance, a value of \samp{0.95} means that only the most intense peaks that together
#'   account for <=95\% of the total intensity are retained. Set to \code{NULL} to ignore.
...
setMethod("filter", "MSPeakLists", function(obj, MSLevel = 1:2, absMinIntensity = NULL, relMinIntensity = NULL,
                                            ...
                                            maxRelCumIntensity = NULL, isolatePrec = NULL, deIsotope = FALSE,
                                            ...
                                            reAverage = FALSE, negate = FALSE)
{
    ...
    checkmate::assertNumber(maxRelCumIntensity, lower = 0, upper = 1, finite = TRUE, null.ok = TRUE, add = ac)
    ...
    hash <- makeHash(obj, MSLevel, absMinIntensity, relMinIntensity, topMostPeaks, minPeaks, maxMZOverPrec,
                     absMinAbundanceFeat, relMinAbundanceFeat, absMinAbundanceFGroup, relMinAbundanceFGroup,
                     maxRelCumIntensity, isolatePrec, deIsotope, removeMZs, withMSMS, annotatedBy, retainPrecursor,
    ...
    obj <- delete(obj, j = function(pl, grp, ana, type)
    {
        ...
        if (type == "MS")
        {
            ...
            plF <- doMSPeakListFilter(plF, absMinIntensity, relMinIntensity, topMostPeaks, NULL, maxMZOverPrec,
                                      ...
                                      maxRelCumIntensity, deIsotope, removeMZs, TRUE, plF[precursor == TRUE]$mz,
                                      mzWindow, negate)
        ...
        else
        {
            ...
            precMZ <- plF[precursor == TRUE]$mz
            if (length(precMZ) == 0)
            {
                # try to get precursor mz from MS peak list instead
                precMZ <- if (!is.null(ana))
                    obj[[ana, grp]]$MS[precursor == TRUE]$mz
                else
            ...
            plF <- doMSPeakListFilter(plF, absMinIntensity, relMinIntensity, topMostPeaks, minPeaks, maxMZOverPrec,
                                      ...
                                      maxRelCumIntensity, deIsotope, removeMZs, retainPrecursor, precMZ, mzWindow,
                                      negate)
        }
...
#'   and their IMS precursors in post mobility workflows (see \code{\link[=assignMobilities_feat]{assignMobilities}}).
#'
...
#'
#' @return \code{spectrumSimilarityIMS} returns a \code{data.table} with spectral similarities for each IMS
#'   precursor and feature pair.
#'
#' @aliases spectrumSimilarityIMS,MSPeakLists-method
#' @aliases spectrumSimilarityIMS
#' @export
setMethod("spectrumSimilarityIMS", "MSPeakLists", function(obj, fGroups, doFGroups = TRUE, warn = TRUE, ...)
{
    if (length(obj) == 0)
        return(NULL)
...
#'
#' Data processing steps that use mass spectral data (\emph{e.g.} \link[=generateFormulas]{formula generation} and
#' \link[=generateCompounds]{compound generation}) typically use 'MS peak lists', which are tables storing the
#' \emph{m/z}, intensity and other data from the raw mass spectra. The \code{generateMSPeakLists} function generates MS
#' and MS/MS peak lists first for all features (or a subset, if the \code{topMost} argument is set). During this step
...
#'   \code{\link{generateMSPeakListsDAFMF}}. These functions are now unmaintained and may not (fully) work anymore. The
...
#'   in the final peak lists. However, annotation and most other functionality typically uses only the (uncombined and)
#'   set specific peak lists, as most algorithms cannot work with mixed peak lists.
#'
#' @return An \code{\link{MSPeakLists}} object.
#'
                                                           ...
                                                           avgFGroupParams = getDefAvgPListParams())
{
    ...
    getMSPL <- function(backend, ft, params, MSLevel)
    {
        ...
        ret <- getMSPeakLists(backend, ft$retmin, ft$retmax, ft$mz, fiw, withPrecursor = params$withPrecursor,
                              ...
                              absMinAbundance = params$absMinAbundance, maxRelCumIntensity = params$maxRelCumIntensity,
