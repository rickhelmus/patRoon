...
{
    checkmate::assertChoice(x, c("openms", "xcms", "xcms3", "kpic2", "greedy"), .var.name = .var.name, add = add)
}
...
assertEIXParams <- function(x, .var.name = checkmate::vname(x), add = NULL)
{
    ...
    assertListVal(x, "window", checkmate::assertNumber, lower = 0, finite = FALSE, .var.name = .var.name, add = add)
...
assertAvgPListParams <- function(x, .var.name = checkmate::vname(x), add = NULL)
{
    ...
    assertListVal(x, "clusterMzWindow", checkmate::assertNumber, lower = 0, finite = TRUE, .var.name = .var.name,
                  add = add)
    assertListVal(x, "relMinAbundance", checkmate::assertNumber, lower = 0, finite = TRUE, .var.name = .var.name,
                  add = add)
...
assertSIRIUSLogin <- function(login, alwaysLogin, add = NULL)
{
    checkmate::assert(
...
{
    assertSIRIUSLogin(login, alwaysLogin, add = add)
    checkmate::assert(
        checkmate::checkPathForOutput(projectPath, overwrite = TRUE, extension = "sirius"),
        checkmate::checkNull(projectPath),
        .var.name = "projectPath", add = add
    )
    checkmate::assertChoice(runMode, c("execute", "read"), add = add)
    checkmate::assertClass(SIRIUSAPI, "rsirius_api", null.ok = TRUE, add = add)
}
...
assertPiekGenEICParams <- function(x, .var.name = checkmate::vname(x), add = NULL)
{
    ...
    assertListVal(x, "smoothWindowMob", checkmate::assertCount, positive = FALSE, .var.name = .var.name, add = add)
    assertListVal(x, "smoothExtMZ", checkmate::assertNumber, lower = 0, finite = TRUE, .var.name = .var.name, add = add)
    assertListVal(x, "smoothExtMob", checkmate::assertNumber, lower = 0, finite = TRUE, .var.name = .var.name, add = add)
    assertListVal(x, "saveMZProfiles", checkmate::assertFlag, .var.name = .var.name, add = add)
