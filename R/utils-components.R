...
omitMobFGroupsForComponentization <- function(fGroups)
{
    if (hasIMS(fGroups))
    {
        printf("NOTE: IMS feature groups will be omitted from componentization\n")
        fGroups <- selectIMSFilter(fGroups, IMS = FALSE, verbose = FALSE)
    }
    return(fGroups)
}

cannotExpandComponIMS <- function(obj)
{
    stop(sprintf("expandForIMS() is not supported for this class of components (%s)", class(obj)), call. = FALSE)
}
