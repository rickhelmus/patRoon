                            ...
                            onlyHits = FALSE)
{
    ...
    if (features)
    {
        ...
        if (!addQualities)
            ...
            ret <- removeDTColumnsIfPresent(ret, getFeatureQualityNames(fGroups, group = FALSE))
        if (!addScores)
            featTab <- removeDTColumnsIfPresent(featTab, getFeatureQualityNames(fGroups, group = FALSE, scores = TRUE))
        else
            ret <- removeDTColumnsIfPresent(ret, getFeatureQualityNames(fGroups, group = FALSE, scores = TRUE))
