...
rGroupLenNonEmpty <- length(unique(analysisInfo(removeEmptyAnalyses(rmdVars$fGroups))$replicate))
rGroupLen <- length(replicates(rmdVars$fGroups))
anyOverlap <- rGroupLen > 1 &&
    length(unique(rmdVars$fGroups, which = replicates(rmdVars$fGroups), outer = TRUE)) < length(rmdVars$fGroups)
...
allPlots <- sapply(seq_len(length(rmdVars$fGroups)), function(grpi)
{
    ...
    makeCachedPlot(path, "plotChroms",
                   list(rmdVars$fGroups, groupName = names(rmdVars$fGroups)[grpi], EICParams = rmdVars$EICParams,
                        retMin = rmdVars$retMin, EICs = rmdVars$EICs, groupBy = "replicate"),
...
plotChroms(rmdVars$fGroups, EICParams = getDefEICParams(window = rmdVars$EICParams$window,
           ...
           groupBy = "fGroups", showLegend = FALSE)
```
...
plot(rmdVars$fGroups, groupBy = "fGroups", showLegend = FALSE, retMin = rmdVars$retMin)
```
...
plotChord(rmdVars$fGroups, aggregate = TRUE)
message("Done!")
```
