...
TPsDT <- rbindlist(Map(cTable, seq_along(cTable), names(cTable), f = function(cmp, cInd, cName)
{
    ...
    trCols <- intersect(c("formula", "retDir", "TP_retDir", "retDiff", "mzDiff", "formulaDiff",
                          "generation", "likelihood", "accumulation", "production", "globalAccumulation",
                          "set", "CID", "SMILES"),
                        names(cmp))
...
{
    glue::glue("\n{ ticks } {{r}}\n",
               "gr <- plotGraph(rmdVars$TPs[{ TPInd }], components = components, structuresMax = rmdVars$TPGraphStructuresMax)\n",
               "gr$elementId <- paste0('TPGraph_', { i })\n",
               "gr\n",
               "{ ticks }\n\n", ticks = "```", i = i, TPInd = match(cInfo$parent_name[i], parents(rmdVars$TPs)$name))
})
