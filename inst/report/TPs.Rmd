...
precInfoCols <- intersect(c("name", "precursor_name", "precursor_group", "precursor_formula", "precursor_CID"), names(cInfo))
precDT[, precursor := sapply(splitCI, function(cir)
{
    precInfo <- cir[, precInfoCols, with = FALSE]
    setnames(precInfo, sub("^precursor_", "", names(precInfo)))
    
    if (!is.null(precInfo[["formula"]]))
        precInfo[, formula := subscriptFormulaHTML(formula)]
    if (!is.null(precInfo[["CID"]]))
        precInfo[, CID := makeDBIdentLink("pubchem", CID)]
...
if (isScreening(rmdVars$fGroups))
{
    ...
    {
        precDT[, screening := sapply(splitCI, function(cir)
        {
            sr <- suspTbl[name == cir$precursor_name & group == cir$precursor_group]
            if (nrow(sr) > 0)
...
TPsDT <- rbindlist(Map(cTable, seq_along(cTable), names(cTable), f = function(cmp, cInd, cName)
{
    ...
    trCols <- intersect(c("formula", "RTDir", "TP_RTDir", "retDiff", "mzDiff", "formulaDiff", "set", "CID"), names(cmp))
    ret[, TP := sapply(splitCmp, function(cr)
    {
        ...
        if (!is.null(tpInfo[["RTDir"]]) && !is.null(tpInfo[["RTDir"]]))
        {
            tpInfo[, "RTDir (predicted/actual)" := paste0(TP_RTDir, "/", RTDir)]
            tpInfo[, c("RTDir", "TP_RTDir") := NULL]
        }
