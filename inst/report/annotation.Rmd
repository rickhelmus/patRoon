...
keepSuspCols <- c("susp_name", "susp_d_rt", "susp_d_mz", "susp_sets", "susp_estIDLevel")
if (isSusp)
{
    fGroupsSpecDT <- as.data.table(rmdVars$fGroups, collapseSuspects = NULL)
    fGroupsSpecDT[, c("susp_d_rt", "susp_d_mz") := .(round(susp_d_rt, 2), round(susp_d_mz, 5))]
    for (col in c("susp_annSimForm", "susp_annSimComp", "susp_annSimBoth", "susp_maxFragMatchesRel"))
    {
    ...
    IDLevInd <- which(keepSuspCols == "susp_estIDLevel")
    keepSuspCols <- c(keepSuspCols[-IDLevInd], "susp_estIDLevel")
} else
