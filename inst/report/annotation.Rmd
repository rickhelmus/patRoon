...
{
    formFGroups <- groupNames(formulas)
...
if (isSusp)
{
    ...
    if (isSet)
        ...
        clR2 <- intersect(clR2, names(fGroupsSpecDT))
...
allPlots <- setNames(lapply(seq_along(plotGroups), function(i)
{
    ...
    if (grp %in% formFGroups)
    {
        ...
        {
            anPList <- annotatedPeakList(formulas, prec, grp,
            ...
            makeCachedPlot(ret, "plotSpectrum", list(formulas, prec, grp, MSPeakLists = rmdVars$MSPeakLists),
...
{
    mn <- mergedConsensusNames(compounds)
...
formulasDT <- rbindlist(lapply(formFGroups, function(grp)
{
    ...
    {
        it <- paste0(getFormInfoList(formulas[[grp]], precursors[preci], mergedConsensusNames(formulas),
                                     useHTML = TRUE), collapse = "<br>")
        if (isSusp)
        {
            # insert suspect names (if any)
            tbl <- as.data.table(rmdVars$fGroups, collapseSuspects = NULL)[group == grp]
    ...
    {
        apl <- annotatedPeakList(formulas, precursor = prec, groupName = grp,
                                 MSPeakLists = rmdVars$MSPeakLists, onlyAnnotated = TRUE)
        if (is.null(apl) || nrow(apl) == 0)
            return("<div align=\"center\">No annotation available.</div>")
