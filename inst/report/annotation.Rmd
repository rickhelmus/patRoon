...
compButtons <- sapply(seq_along(allGroups), function(gi) maybeAddButton(gi, compFGroups, "compounds", "showAnnotation", gi, "comp"))
...
```{r echo=FALSE, length(plotGroups) > 0}
compoundsDT <- rbindlist(lapply(seq_along(plotGroups), function(grpi)
{
    grp <- plotGroups[grpi]
...
`r if (FALSE && length(plotGroups) > 0) rmdText`
