...
allPlots <- lapply(seq_along(plotGroups), function(grpi)
{
    ...
    unAnnotatedMSSpec <- file.path(plotPathFull, sprintf("spec_%d.png", grpi))
    ...
    if (grp %in% compFGroups)
    {
        ...
        grpPlots[["compoundScores"]] <- sapply(compsSeq, function(compi)
        {
            ...
            makePlot(ret, 4.5, 4.5, bg = NA,
                     plotScores(rmdVars$compounds, compi, grp, rmdVars$compoundNormalizeScores,
        ...
        grpPlots[["compoundSpectra"]] <- sapply(compsSeq, function(compi)
        {
            ...
            makePlot(ret, 7, 4.5, bg = NA, plotSpec(rmdVars$compounds, compi,  grp,
        ...
        grpPlots[["compoundStructs"]] <- sapply(compsSeq, function(compi)
        {
            ...
            makePlot(ret, 3, 3, bg = NA, plotStructure(rmdVars$compounds, compi, grp, width = 150, height = 150))
            return(ret)
        })
    }
    ...
    if (grp %in% compsClustFGroups)
    {
        ...
        makePlot(plotf, 8, 4.5, plot(rmdVars$compsCluster, groupName = grp))
        ...
        grpPlots[["compClustMCS"]] <- sapply(seq_along(unique(ct)), function(cli)
        {
            ...
            makePlot(ret, 3, 3, plotStructure(rmdVars$compsCluster, grp, cli,
                                              100, 100))
            return(ret)
        })
    }
    ...
    if (grp %in% formFGroups)
    {
        ...
        grpPlots[["formulaScores"]] <- sapply(precursors, function(prec)
        {
            ...
            makePlot(ret, 4.5, 4.5, bg = NA,
                     plotScores(rmdVars$formulas, prec, grp,
        ...
        grpPlots[["formulaSpecs"]] <- sapply(precursors, function(prec)
        {
            ...
            makePlot(ret, 6, 4.5, plotSpec(rmdVars$formulas, prec, grp, MSPeakLists = rmdVars$MSPeakLists))
            return(ret)
        })
    }
...
if (rmdVars$optimizePng && length(ap > 0))
    optimizePngPlots(ap, stderr(), rmdVars$maxProcAmount)
...
rmdText <- sprintf("<script>var EICPaths = [ %s ];</script>",
                   paste0("'", sapply(allPlots, "[[", "EIC"), "'", collapse = ", "))
```
