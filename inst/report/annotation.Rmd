...
# for compound/formula tables
imgSrcText <- if (rmdVars$selfContained) "src=" else "src=file://"
...
if (!is.null(rmdVars$formConsensus) && "formulas" %in% rmdVars$reportPlots)
{
...
for (grpi in seq_along(plotGroups))
{
    ...
    if (plotGroups[grpi] %in% compFGroups)
    {
        ...
        allPlots <- c(allPlots, sapply(compsSeq, function(compi)
        {
            ...
            makePlot(ret, 4.5, 4.5, bg = NA, plotScores(rmdVars$compounds, compi, plotGroups[grpi]))
            return(ret)
        }))
...
compoundsDT <- rbindlist(lapply(compFGroups, function(grp)
{
    ...
    return(data.table(group = grpi,
                      "#" = seq_len(nrow(ct)),
                      compound = sprintf("<img %s%s></img><br>%s", imgSrcText, stpaths, infoTexts),
                      spectrum = sprintf("<img %s%s></img>", imgSrcText, sppaths),
                      scores = sprintf("<img %s%s></img>", imgSrcText, scpaths)))
}))
