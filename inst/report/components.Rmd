...
makeCompDT <- function(s)
{
    ...
    initDT <- DT::JS("function(settings, json) {",
                     "setTimeout(function() {",
                     sprintf("$(\"#componentsTable_%s .dataTable\").DataTable().columns.adjust().draw(); }", s),
                     ", 25); }")
    DT::datatable(compTable, options = list(scrollX = TRUE, scrollY = "600px", deferRender = TRUE,
                                            dom = "lrtip", pageLength = 25, autoWidth = FALSE,
                                            initComplete = initDT, ordering = FALSE),
                  class = "striped row-border", elementId = paste0("componentsTable_", s),
                  fillContainer = TRUE, rownames = FALSE, escape = FALSE)
}

hd <- paste0("### %s { .components }\n\n",
             "NOTE: only components with feature group data are shown here.\n\n")

if (isSet)
{
    rmdText <- "## { data-height=800 .components .tabset }\n\n"
    for (s in sets(rmdVars$components))
    {
        rmdText <- c(rmdText, paste0(sprintf(hd, s),
                                     glue::glue("
{ ticks } {{r}}
makeCompDT('{ s }')
{ ticks }\n\n", ticks = "```", s = s)))
...
{
    rmdText <- paste0("## { data-height=800 .components }\n\n",
                      sprintf(hd, "Components"),
                      glue::glue("
{ ticks } {{r}}
makeCompDT(NULL)
{ ticks }", ticks = "```"))
