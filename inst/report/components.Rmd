...
isTP <- inherits(rmdVars$components, "componentsTPs")
...
```{r echo=FALSE,eval=isHomol || isTP}
rmdText <- knitr::knit(text = glue::glue("
##
...
`r if (isHClust || isHomol || isTP) rmdText`
...
makeCompDT <- function(s, scrollY)
{
    ...
    if (isTP)
    {
        cmpInfo <- componentInfo(rmdVars$components)
        compTable[, component := sprintf("%s<br>(prec: %s - group: %s)", component, cmpInfo$precursor_name,
                                         cmpInfo$precursor_group)]
    }
