...
if (length(compFGroups))
    fGroupsSpecDT[group %in% compFGroups, compound := showButton("showSpec", group, "comp")]
if (length(formFGroups))
    fGroupsSpecDT[group %in% formFGroups, formula := showButton("showSpec", group, "form")]
...
## { grp } {{ .specClass .EIC-{ grp } }}

### EIC {{ .specClass .EIC-{ grp } }}

![]({ plotPath }/eic_{ grpi }.png)

", grpi = grpi, grp = specGroups[grpi], plotPath = plotPath)
    ...
    {
        precursors <- unique(formTable[group == specGroups[grpi], formula])
        rmdTexts[grpi] <- paste(rmdTexts[grpi], paste0(glue::glue(formSpecTempl, grpi = grpi, grp = specGroups[grpi],
