...
workPath <- "<WORK_PATH>/test-np/features-norm_istd_sets"
setwd(workPath)
...
# Load ISTD list
ISTDListPos <- read.csv("istd-pos")
ISTDListNeg <- read.csv("istd-neg")
...
fGroups <- normInts(fGroups, featNorm = "istd", groupNorm = FALSE, normFunc = max,
                    standards = c(ISTDListPos, ISTDListNeg), ISTDRTWindow = 120, ISTDMZWindow = 300, minISTDs = 3)
