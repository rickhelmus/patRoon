...
workPath <- "test_temp/test-np/tp-cts_scr"
setwd(workPath)
...
# NOTE: onlyHits is set to FALSE to ensure TPs can be found below
# Set adduct to NULL if suspect list contains an adduct column
fGroups <- screenSuspects(fGroups, suspList, adduct = "[M+H]+", onlyHits = FALSE)
...
# Obtain TPs
TPs <- generateTPs("cts", parents = fGroups, transLibrary = "photolysis_ranked", generations = 1,
                   TPStructParams = getDefTPStructParams())

# Screen TPs
suspListTPs <- convertToSuspects(TPs, includeParents = FALSE)
# Amend with TP screening
fGroups <- screenSuspects(fGroups, suspListTPs, onlyHits = TRUE, amend = TRUE)
