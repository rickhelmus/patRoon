...
test_that("plotting works", {
    expect_docker("retmz", function() plot(fgOpenMS))
    expect_docker("retmz-comp", function() plot(fGCompOpenMS))
    
    expect_docker("intensity-def", function() plotInt(fgOpenMS))
    expect_docker("intensity-avg", function() plotInt(fgOpenMS, TRUE))
    
    expect_docker("chord-def", function() plotChord(fgOpenMS))
    expect_docker("chord-selflinks", function() plotChord(fgOpenMS, addSelfLinks = TRUE))
    expect_docker("chord-nortmz", function() plotChord(fgOpenMS, addRetMzPlots = FALSE))
    expect_docker("chord-comp", function() plotChord(fGCompOpenMS))
    
    expect_docker("eic-def", function() plotEIC(subFGroups))
    expect_docker("eic-rtmin", function() plotEIC(subFGroups, retMin = TRUE))
    expect_docker("eic-tm1", function() plotEIC(subFGroups, topMost = 1))
    expect_docker("eic-area", function() plotEIC(subFGroups, showPeakArea = TRUE))
    expect_docker("eic-cbr", function() plotEIC(subFGroups, colourBy = "rGroups"))
    expect_docker("eic-cbf", function() plotEIC(subFGroups, colourBy = "fGroups"))
    expect_docker("eic-ann", function() plotEIC(subFGroups, annotate = "mz"))
    
    expect_docker("venn", function() plotVenn(fgOpenMS))
    expect_docker("venn-comp", function() plotVenn(fGCompOpenMS))
})
