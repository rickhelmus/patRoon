...
testMSPLCumInt <- function(obj, orig, avg, thr, negate = FALSE)
{
    tab <- as.data.table(obj, averaged = avg)
    
    tabOrig <- as.data.table(orig, averaged = avg)
    by <- c("group", "set", "type")
    if (!avg)
        by <- c(by, "analysis")
...
test_that("avg params", {
    ...
    testMSPLAbundance(generateMSPeakLists(fGroups, avgFeatParams = getDefAvgPListParams(relMinAbundance = 0.5)), FALSE, TRUE, 0.5)
    testMSPLAbundance(generateMSPeakLists(fGroups, avgFeatParams = getDefAvgPListParams(absMinAbundance = 10)), FALSE, FALSE, 10)
...
test_that("delete and filter", {
    ...
    expect_min_gte(getMSPLProp(filter(plists, relMinAbundanceFeat = 0.5), "abundance_rel", avg = FALSE), 0.5)
    expect_min_gte(getMSPLProp(filter(plists, absMinAbundanceFeat = 10), "abundance_abs", avg = FALSE), 10)
    expect_max_lt(getMSPLProp(filter(plists, relMinAbundanceFeat = 0.5, negate = TRUE), "abundance_rel", avg = FALSE), 0.5)
    expect_max_lt(getMSPLProp(filter(plists, absMinAbundanceFeat = 10, negate = TRUE), "abundance_abs", avg = FALSE), 10)
    
    expect_min_gte(getMSPLProp(filter(plists, relMinAbundanceFeat = 0.5), "feat_abundance_rel", avg = TRUE), 0.5)
    expect_min_gte(getMSPLProp(filter(plists, absMinAbundanceFeat = 10), "feat_abundance_abs", avg = TRUE), 10)
    expect_max_lt(getMSPLProp(filter(plists, relMinAbundanceFeat = 0.5, negate = TRUE), "feat_abundance_rel", avg = TRUE), 0.5)
    expect_max_lt(getMSPLProp(filter(plists, absMinAbundanceFeat = 10, negate = TRUE), "feat_abundance_abs", avg = TRUE), 10)
    
    expect_min_gte(getMSPLProp(filter(plists, relMinAbundanceFGroup = 0.5), "fgroup_abundance_rel", avg = TRUE), 0.5)
    expect_min_gte(getMSPLProp(filter(plists, absMinAbundanceFGroup = 2), "fgroup_abundance_abs", avg = TRUE), 2)
...
test_that("IMS tests", {
    ...
    specSims <- spectrumSimilarityIMS(plistsIMS, fGroupsIMS, doFGroups = TRUE)
    checkmate::expect_data_table(specSims)
    checkmate::expect_names(names(specSims), identical.to = c("group", "ims_precursor_group", "similarity"))
    expect_range(specSims$similarity, c(0, 1))
    fgs <- groupNames(plistsIMS)
    # omit fgs without IMS features (UNDONE: make a filter?)
    fgs <- intersect(unlist(groupInfo(fGroupsIMS[IMS = TRUE])[, c("group", "ims_precursor_group"), with = FALSE]), fgs)
    expect_setequal(c(unique(specSims$group), unique(specSims$ims_precursor_group)), fgs)
    
    specSimsFeats <- spectrumSimilarityIMS(plistsIMS, fGroupsIMS, doFGroups = FALSE)
    checkmate::expect_data_table(specSimsFeats)
    checkmate::expect_names(names(specSimsFeats), identical.to = c("group", "ims_precursor_group", "analysis", "similarity"))
    ...
    expect_setequal(c(unique(specSimsFeats$group), unique(specSimsFeats$ims_precursor_group)), fgs)
    
    expect_null(spectrumSimilarityIMS(delete(plistsIMS), fGroupsIMS))
    expect_warning(spectrumSimilarityIMS(plistsIMS, delete(fGroupsIMS)), "No relevant")
    expect_warning(spectrumSimilarityIMS(plistsIMS, delete(fGroupsIMS), warn = FALSE), NA)
    expect_error(spectrumSimilarityIMS(plistsIMS, fGroupsIMS[, IMS = FALSE]), "No mobility")
    expect_warning(spectrumSimilarityIMS(plistsIMS, fGroupsIMS[, IMS = TRUE]), "No relevant")
})
...
test_that("BG MSMS", {
    ...
    expect_min_gte(doGetBG(avgSpectraParams = getDefAvgPListParams(relMinAbundance = 0.5, topMost = 25))$abundance_rel_spec, 0.5)
    expect_min_gte(doGetBG(avgSpectraParams = getDefAvgPListParams(absMinAbundance = 50, topMost = 25))$abundance_abs_spec, 50)
    expect_min_gte(doGetBG(avgAnalysesParams = getDefAvgPListParams(relMinAbundance = 0.5, topMost = 25))$abundance_rel_ana, 0.5)
    expect_min_gte(doGetBG(avgAnalysesParams = getDefAvgPListParams(absMinAbundance = 2, topMost = 25))$abundance_abs_ana, 2)
})
