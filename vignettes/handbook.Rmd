...
> **_NOTE_** If you are not familiar with `data.table` see [data.table]. Briefly, this is a more efficient and largely compatible alternative to the regular `data.frame`.

> **_NOTE_** Internally `as.data.frame()` simply calls `as.data.table()`, for this reason, the reference documentation only lists the former.
...
as.data.table(formulas, maxFormulas = 1, OM = TRUE)
