...
The previous sections mainly discussed how to create workflow data. The next sections will discuss how to _use_ the data.
...
`as.data.table()` / `as.data.frame()`               | All                           | Convert data to a `data.table` or `data.frame`.
`analysisInfo()`, `analyses()`, `replicateGroups()` | `features`, `featureGroups`   | Returns the [analysis information](#anaInfo), analyses and replicate groups for which this object contains data.
...
The common `R` extraction operators `"[["`, `"$"` can be used to obtain data for a particular feature groups, analyses etc:
