...
<!-- UNDONE: update when XCMS3 support is available -->

The `getXCMSSet()` function converts a `features` or `featureGroups` object to an `xcmsSet` object which can be used for further processing with [xcms] (the new XCMS3 interface is not yet supported).
...
<!-- UNDONE: point to some useful cache file, possibly update next example, partial matching -->
