...
Firstly, if MS peak lists are provided, then the [spectrum similarity](#specSim) is calculated between each parent and its potential TP candidates. This is performed with all the three different alignment shifts (see the [spectrum similarity section] for more details).
...
3. A second suspect screening is performed with both parents and TPs. Note that the parent data is needed for componentization.
...
```{r eval=FALSE}
# step 1
fGroupsParents <- screenSuspects(fGroups, patRoonData::suspectsPos, onlyHits = TRUE)
# step 2
TPs <- generateTPs("library", parents = fGroupsParents)
...
compoundsScr <- generateCompounds(fGroupsScr, mslistsScr, "metfrag", database = "csv",
                                  extraOpts = list(LocalDatabasePath = "TP-database.csv"))
...
# step 6
componTP <- generateComponents(fGroupsScrAnn, "tp", MSPeakLists = mslistsScr,
                               compunds = compoundsScr)
fGroupsScrAnn <- fGroupsScrAnn[, union(componentInfo(componTP)$parent_group,
                                       groupNames(componTP))]
...
4. A suspect screening is performed to find the TPs. Unlike the previous example the parents are not screened, and feature groups without hits are kept ([discussed here](#TPsFGroups)).
...
formulas <- generateFormulas(fGroups, "genform", adduct = "[M+H]+")
formulas <- filter(formulas, topMost = 5)
fGroups <- fGroups[, groupNames(formulas)]
...
# step 5
componTP <- generateComponents(fGroups, "tp", MSPeakLists = mslists, formulas = formulas)

# step 6
componTP <- filter(componTP, retDirMatch = TRUE, formulas = formulas)

# step 7
fGroups <- fGroups[, union(componentInfo(componTP)$parent_group, groupNames(componTP))]
