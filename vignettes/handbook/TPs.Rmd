...
componTP <- generateComponents(algorithm = "tp",
                               fGroups = fGroups[replicate = "before"],
                               fGroupsTPs = fGroups[replicate = "after"])
```

In this example, only those feature groups present in the "before" replicate are considered as parents, and those in "after" may be considered as a TP. Since it is likely that there will be some overlap in feature groups between both sample groups, the `ignoreParents` flag can be used to not consider any of the overlap for TP assignments:
