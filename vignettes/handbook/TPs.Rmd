...
```{r eval=FALSE}
# step 1
fGroupsScr <- screenSuspects(fGroups, patRoonData::suspectsPos)
# step 2
TPs <- generateTPs("library", parents = fGroupsScr)

# step 3
suspects <- convertToSuspects(TPs)
fGroupsScr <- screenSuspects(fGroupsScr, suspects, onlyHits = TRUE, amend = TRUE)
...
# step 5
componTP <- generateComponents(fGroupsScr, "tp", MSPeakLists = mslists, formulas = formulas)
