...
The maximum number of threads used for parallelization is configured through the `patRoon.threads` option. By default it is set to the number of available CPU cores, which results usually in the best performance. However, this could be lowered, for instance, to keep your computer more responsive while processing.
...
`TIMSCONVERT`         | `convertMSFiles(algorithm="timsconvert", ...)` | Only if `useTimsTOF=TRUE`
