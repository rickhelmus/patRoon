...
In [IMS workflows](#IMSWorkflow) with post mobility assignment `normInts()` will not consider any mobility features for the assignment of internal standards and calculation of TICs and the normalized intensity values for mobility features will be copied from IMS parents. 
...
> **_NOTE_** None of the exporting and conversion functions support data from [sets workflows](#setsWorkflow) or [IMS workflows](#IMSWorkflow). For this reason, the `set` argument must be given to specify for which features should be exported in sets workflows, and any mobility features will _not_ be exported.
...
>**_NOTE_** The `getQuantCalibFromScreening()` function ignores any mobility features in [IMS workflows](#IMSWorkflow) with post mobility assignment, as the calibration procedure does not take any differences due to IMS separation into account.
