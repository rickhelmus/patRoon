...
As can be seen from above, labelled isotopes can be specified with square brackets, _e.g._ [2H] for deuterium.
...
a. Suspect hits that were obtained with `screenSuspects` [link].
...
The calibrant data is used by `MS2Quant` to convert predicted ionization efficiencies to actual response factors, which are specific to the used LC instrument and methodology. For this purpose, several mixtures with known concentrations (i.e. standards) should be measured alongside your samples. The easiest way then to obtain the calibrant data is by perform a 'suspect screening' [link] with the calibrant compounds and subsequently using the `getQuantCalibFromScreening()` function to automatically extract the relevant data needed for calibration. This function requires you to give the concentration data for each calibrant in order to produce the final calibrant data. An example is shown below:

```{r eval=runData}
```

The next step is to obtain the eluent information. This is simple a `data.frame` that describes the LC gradient program as time _versus_ percentage of B (i.e. the organic modifier).

```{r eval=runData}
```

The following example shows an example workflow to use this data to predict response factors and subsequently calculate concentrations for all features.

### Inspecting and using predicted values
