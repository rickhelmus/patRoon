...
# Calculate toxicity for compound hits. Limit to the top 5 to limit calculation time.
...
calibrants <- merge(calibrants, fgTab[, c("susp_name", "susp_rt", "standard-pos"), with = FALSE], by.x = "name", by.y = "susp_name")
setnames(calibrants, c("susp_rt", "standard-pos"), c("rt", "intensity"))
...
The concentrations are specified separately for each calibrant compound. The column names should follow the names of the [replicate groups assigned to the standards](#anaInfo).
...
fGroupsSuspTox <- predictRespFactors(
    fGroupsSusp,
...
fGroupsSuspTox <- calculateConcs(fGroupsSuspTox, areas = FALSE)
```
