...
`pngquant`            | `reportHTMLLegacy(...)`                      | Only if `optimizePng=TRUE`
...
Which method is used is controlled by the `patRoon.MP.method` package option. Note that `reportHTMLLegacy()` will always use the classic method for `pngquant`.
