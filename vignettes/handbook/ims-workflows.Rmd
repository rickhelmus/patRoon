...
The `piek` algorithm was already [discussed before](#piek). By default, `piek` does not use IMS data for feature detection, even if processing IMS-HRMS data. To enable the use of IMS data, the `IMS` argument of `getPiekEICParams` must be set, either to `"bruker"` or `"agilent"`. The `filterIMS` EIC parameter can be set to filter EIC bins by mobility data, similarly as the `filter` parameter is used to filter the bins by _m/z_. The following combinations of `filter` and `filterIMS` are supported:

`filter`     | `filterIMS`  | EIC formation
------------ | ------------ | -----------------------------------------------------------------------
`"none"`     | `"none"`     | no filtering.
`"suspects"` | `"suspects"` | filter by _m/z_ and mobility data in a suspect list.
`"suspects"` | `"none"`     | only filter by suspect _m/z_ data.
`"ms2"`      | `"ms2"`      | only keep bins with _m/z_ and mobility data the precursors detected in Bruker DDA-PASEF MS2 experiments.
`"ms2"`      | `"none"`     | only filter by precursor _m/z_ data.

Some examples are given below:

```{r exDirectMob, eval=FALSE}
# comprehensive feature detection from 2 dimensional bins
genEICParams <- getPiekEICParams(IMS = "bruker", mzRange = c(100, 300), mobRange = c(0.5, 1.5), mzStep = 0.02, mobStep = 0.2)
...
genEICParams <- getPiekEICParams(IMS = "bruker", filter = "ms2", filterIMS = "ms2", minTIC = 1000)
fList <- findFeatures(anaInfo, "piek", genEICParams = genEICParams, peakParams = peakParams)

# only filter by suspect m/z data (e.g. if suspect list doesn't contain IMS data)
genEICParams <- getPiekEICParams(IMS = "bruker", filter = "suspects", filterIMS = "none", mobRange = c(0.5, 1.5))
fList <- findFeatures(anaInfo, "piek", genEICParams = genEICParams, peakParams = peakParams,
                      suspects = suspList, adduct = "[M+H]+")
```

> **_NOTE_** without EIC filtering feature detection can become quite computationally intensive. There are many advanced EIC paramaters that can be tweaked to improve the performance, see `?getPiekEICParams` for more details.

The grouping of features in direct mobility assignment workflows is currently only possible with the `greedy` algorithm or by [importing data](#importFeat). The `greedy` algorithm was already [briefly introduced before](#groupFeatures). This algorithm was introduced in `patRoon 3.0` and is a simple and fast grouping algorithm, which also supports IMS data. Its `IMSWindow` argument can be set to configure the mobility grouping tolerance, for instance:

```{r exGroupGreedyIMS, eval=FALSE}
fGroups <- groupFeatures(fList, "greedy", IMSWindow = 1)
...
The following subsections further discuss the two purposes of `assignMobilities()` in more detail. Further details are found in the reference manual (`?assignMobilities_feat`).
...
fGroups <- assignMobilities(fGroups, fromSuspects = TRUE, IMSWindow = 0.01,
...
fGroups <- assignMobilities(fGroups, fromSuspects = TRUE, IMSWindow = 0.1,
