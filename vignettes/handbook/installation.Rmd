...
Start RStudio --> Tools --> Global options --> General tab --> R version --> Change

Manually set the `R` version by selecting `Rterm.exe` from the `R/bin/x64` directory in the bundle (see screenshot below).
...
* `patRoonDeps`: contains `patRoon` and its dependencies (including their dependencies) with versions that were tested against the latest `patRoon` version. This repository is used for the [patRoon bundle](#pBundle), and only available for on Windows systems.
...
```{r eval=FALSE}
install.packages("remotes") # run this remotes is not already installed
...
> **_NOTE_** Currently, `patRoonExt` does not install ProteoWizard due to license restrictions, and some tools, such as [OpenMS] and [OpenBabel], are only installed on Windows systems. See the next section to install these tools.

##### Manually installing and configuring external tools {#inst_man_ext}

For a full manual installation, please download the tools manually from the linked sources shown in the table above. After installation, you may need to configure their filepaths (OpenMS and ProteoWizard are usually found automatically). To configure the file locations you should set some global package options with the `options()` R function, for instance:
...
{
    k <- knitr::kable(dt, format = "latex", booktabs = TRUE, escape = FALSE)
...
install.packages("KPIC", repos = 'https://rickhelmus.r-universe.dev')

# Install the mandatory CAMERA package (only needed if not using patRoonDeps/r-universe)
install.packages("BiocManager") # execute this if 'BiocManager' is not yet installed
BiocManager::install("CAMERA")

# Install patRoonData from GitHub
install.packages("remotes") # execute this if 'remotes' is not yet installed
