...
docker run --rm -it patroonorg/patroon

# run a linux shell, from which R can be launched
docker run --rm -it patroonorg/patroon bash

# run rstudio server, acessible from localhost:8787
# login with rstudio/yourpasswordhere
docker run --rm -p 8787:8787 -u 0 -e PASSWORD=yourpasswordhere -v ~/myvolume:/home/rstudio patroonorg/patroon /init
```

Note that the first two commands run as the default user `patRoon`, while the last as `root`. The last command launches [RStudio] server. You can access it by browsing to `localhost:8787` and logging with user `rstudio` and the password defined by the `PASSWORD` variable from the command (`yourpasswordhere` in the above example). This command also links a local volume to the `rstudio` user in order to obtain persistence of files in the container's home directory. The Docker image is based on the excellent [RStudio image] from the [rocker project](https://www.rocker-project.org/). For more information on RStudio related options see the documentation for the [RStudio image].
