...
`plotEICs()`      | `featureGroups`, `components`                        | Plot extracted ion chromatograms (EICs)
...
The most common plotting functions are `plotEICs()`, which plots chromatographic data for features, and `plotSpectrum()`, which will plot (annotated) spectra. An overview of their most important function arguments are shown below.

Argument                                      | Generic                   | Remarks
--------------------------------------------- | ------------------------- | -------------------------------------------------------------------
`rtWindow`                                    | `plotEICs()`                   | Extra time (in s) +/- retention limits of plotted features (useful to zoom out)
`mzWindow`                                    | `plotEICs()`                   | m/z width of EICs (in Da)
`retMin`                                      | `plotEICs()`                   | If `TRUE` plot retention times in minutes
`topMost`                                     | `plotEICs()`                   | Only draw this amount of highest intensity features in each group.
`showPeakArea`, `showFGroupRect`              | `plotEICs()`                   | Fill peak areas / draw rectangles around feature groups?
`title`                                       | `plotEICs()`, `plotSpectrum()` | Override plot title
`colourBy`                                    | `plotEICs()`                   | Colour individual feature groups (`"fGroups"`) or replicate groups (`"rGroups"`). By default nothing is coloured (`"none"`)
`showLegend`                                  | `plotEICs()`                   | Display a legend? (only if `colourBy!="none"`)
`onlyPresent`                                 | `plotEICs()`                   | Only plot EICs for analyses where a feature was detected? Setting to `FALSE` is useful to inspect if a feature was 'missed'.
`xlim`, `ylim`                                | `plotEICs()`, `plotSpectrum()` | Override x/y axis ranges, i.e. to manually set plotting range.
...
```{r plotEICsSub,eval=runData}
plotEICs(fGroups[1:2]) # only plot EICs from first and second analyses.
plotEICs(fGroups[, 1]) # only plot all features of first group
```
...
plotEICs(fGroups) # plot EICs for all features
...
plotEICs(components, index = 7, fGroups = fGroups) # EICs from a component
...
The `plotEICs()` function introduced before can be used to visualize chromatography data for one or more features. An interactive alternative is to call the `checkChromatograms()` function. This function will launch a GUI that allows you to browse through all features and inspect their EICs. Simply pass in the `featureGroups` object you want to inspect:
