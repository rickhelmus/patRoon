...
The reports are generated by the `reportHTML()` function, which accepts various data objects from the workflow. The output is an `HTML` file, which can be opened with a regular web browser, and allows you to interactively explore and visualize the data.
...
More advanced arguments to `reportHTML()` are discussed in the reference manual (`?reporting`).
