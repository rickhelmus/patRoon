...
The reports are generated by the `reportHTML()` function, which combines the data generated during the workflow. The output `HTML` file can be opened with a regular web browser, and allows you to interactively explore and visualize the data. The reports combine chromatograms, mass spectra, tables with feature and annotation properties and many other useful ways to easily explore your data.
...
```yaml
`r readAllFile(system.file("report", "settings.yml", package = "patRoon"))`
```
...
* `reportHTMLLegacy()`: generates interactive and easily explorable reports

Like the `reportHTML()` function described above, the arguments to these functions control which data will be reported. However, these functions do not use a report settings file, and all configuration happens through function arguments. Some common arguments are listed below; for a complete listing see the reference manual (`` ?`reporting-legacy` ``).
...
`MSPeakLists`                         | `reportPDF()`, `reportHTMLLegacy()` | The `MSPeakLists` object that was used to generate annotation data. Only needs to be specified if `formulas` or `compounds` are reported.
`path`                                | All                           | Directory path where report files will be stored (`"report"` by default).
`formulasTopMost`, `compoundsTopMost` | `reportPDF()`, `reportHTMLLegacy()` | Report no more than this amount of highest ranked candidates.
`selfContained`                       | `reportHTMLLegacy()`          | Outputs to a single and self contained `.html` file. Handy to share reports, but not recommended for large amounts of data.

Some typical examples:

```{r reporting,eval=FALSE}
reportHTMLLegacy(fGroups) # simple interactive report with feature data
...
reportHTMLLegacy(fGroups, formulas = formulas, compounds = compounds,
