...
## Inspecting results {#procInspect}
...
`MSPeakLists`   | analyses       | feature groups | set `reAverage=TRUE` to re-average peak lists after subsetting
...
`featureGroupsScreening` | analysis      | feature group    | suspect (can be `NA`, i` must be `NULL`)
`MSPeakLists`            | feature group | mass peak        | analysis (if set, no deletion occurs on group-averaged peak lists)
...
Some additional parameters exist to the functions described in this section. As usually check the reference manual for more details (_e.g._ `?checkFeatures`).
...
## Updating feature group data {#updFG}

The feature group properties such as retention time and _m/z_ are calculated during the feature grouping process. These properties usually represent an average calculated from the features within the group, and are e.g. used to match [suspects](#suspscr). It may be useful to recalculate these values after e.g. filtering noisy features to to improve the accuracy. This is achieved with the `updateGroups` function:
...
`aggregate`  | `unique()`, `overlap()` | What should be compared: analyses (`aggregate=FALSE`), replicates (`aggregate=TRUE`) or other groups of analyses based on their metadata ([discussed later](#useMeta)).
...
`outer`      | `unique()`              | If `TRUE` then only return features which are _also_ unique among the compared data selected in `which`.
`exclusive`  | `overlap`               | Only keep features that _only_ overlap between the compared data selected by which.
...
Note that several plotting functions that are [discussed in the visualization section](#visComp) can also assist in determining overlap and uniqueness of features.
...
`plot()`          | `componentsClust`                                    | Creates a dendrogram for the clustered components
`plotGraph()`     | `componentsNT`, `componentsNet`                      | Visualization linked homologous series or network of components
...
plotInt(fGroups[, 1:10], groupBy = "fGroups") # plot intensities of first ten feature groups and group them
...
plotGraph(componNT)
plotGraph(componNet, analysis = "standard-pos-1") # inspect network of components
```
...
plotVenn(fGroups, which = c("repl1", "repl2"), aggregate = TRUE) # compare some replicates
...
## Use and modify sample analysis metadata {#useMeta}

The [analysis information](#anaInfo) section briefly introduced the incorporation of sample metadata. For instance, consider the following analysis information for a degradation experiment:

```{r anaInfoMeta,eval=runData}
...
```{r fGroupsMeta,eval=runData,include=FALSE}
fGroupsMeta <- fGroupsRep
analysisInfo(fGroupsMeta) <- anaInfoMeta
```

As [briefly mentioned before](#procInspect), the `analysisInfo()` function can be used query the analysis information of a workflow object.

```{r aiMeta,eval=runData}
...
```{r ADTMetaCols,eval=runData}
as.data.table(fGroupsMeta, features = TRUE, anaInfoCols = c("experiment", "exposure"))[1:5, .(group, analysis, anaInfo_experiment, anaInfo_exposure)]
```
...
---------------------------------------------------- | ---------- | ------------------------------------------------------------------------
`as.data.table()`, `plotInt()`                       | `average`  | Averages intensities from analyses grouped by metadata.
`unique()`, `overlap()`, `plotVenn()`, `plotUpSet()`, `plotChord()` | `aggregate` | Compares analysis groups defined by metadata.
`plot()`, `plotInt()`, `plotChord()`, `plotChroms()` | `groupBy`  | Groups data in plots.

Some examples are shown below.

```{r metaBy,eval=runData}
as.data.table(fGroupsMeta, average = "experiment")[1:5] # average intensities by experiment
plotVenn(fGroupsMeta, aggregate = "experiment") # compare feature presence by experiment
plotChord(fGroupsMeta, aggregate = FALSE, groupBy = "experiment") # group feature overlap by experiment
plotChroms(fGroupsMeta[, 320], groupBy = "experiment", showLegend = TRUE) # EICs grouped by experiment
```
...
```{r AnaReorder,eval=runData,out.width="50%",fig.show="hold"}
...
> **_NOTE_** The analysis information is internally stored as a `data.table`. Do not attempt to modify it directly with the reference assignment operator (`:=`) of `data.table`. Instead, alter a copy and replace the table, as demonstrated by the second example above.
