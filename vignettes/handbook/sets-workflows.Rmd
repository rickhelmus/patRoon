...
> **_NOTE_** The `analysis` names for the [analysis information](#anaInfo) must be _unique_ for each row, even among sets. Furthermore, replicates should not contain analyses from different sets.
...
In sets workflows the [analysis information](#anaInfo) is amended with a `set` column to specify the set each analysis belongs to. Just like [other columns](#useMeta) in the analysis information, the `set` column can be used to group and aggregate data:

```{r setsProcMeta,eval=runData}
...
### Initiating a sets workflow from feature groups {#makeSetFG}
