...
fListNeg <- findFeatures(anaInfoPos, "openms")
...
This workflow will do all the steps for positive _and_ negative data. Only a few modifications were necessary:
...
* section link
* mention this section is mostly theoretical
...
All data objects that are generated during a sets workflow _inherit_ from the classes from a 'regular' workflow. This means that, with some minor exceptions, _all_ of the data processing functionality discussed in the [previous chapter] is also applicable to a sets workflow.
...
fGroupsPos <- fGroups[, sets = "positive"] # only keep feature groups that have positive data
fGroupsF <- filter(fGroups, relMinSets = 1) # only keep feature groups that have feature data for all sets
...
The `makeSet` function also be used to initiate a sets workflow from feature groups:
...
fGroups <- makeSet(fGroupsPos, fGroupsNeg, groupAlgo = "openms", adducts = c("[M+H]+", "[M-H]-"))

# do rest of the workflow...

```

In this case `makeSet` combines the positive and negative features, neutralizes them and re-groups them all together (with the algorithm specified by `groupAlgo`).
...
### Inspecting set objects

### Conversion to regular workflows
