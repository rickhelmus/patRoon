...
if (runData)
{
    ...
    fGroupsAnn <- screenSuspects(fGroups, patRoonData::suspectsPos[patRoonData::suspectsPos$name %in% c("1H-benzotriazole", "N-Phenyl urea",
                                                                                                        "2-Hydroxyquinoline", "DEET"), ], onlyHits = TRUE)
    ...
    saveRDS(list(fList = fList, fGroups = fGroups, fGroupsUF = fGroupsUF, fListRG = fListRG,
                 ...
                 ftOpt = if (doOpt) ftOpt else NULL),
            "~/handbook-obj.Rds")
}
```
