...
Most algorithms in non-target workflows exclusively operate on centroided data files. In most cases the centroided data should be stored in the `mzML` format, but [some feature detection algorithms](#findFeat) only work with `mzXML`. The raw data files are primarily used for data conversion to any of the other types. The raw and ims data file types are (also) used in [IMS workflows](#IMSWorkflow).
...
`piek`           | `findFeatures(algorithm = "piek", ...)`     | raw data loaded with [msdata] | Introduced in `patRoon 3.0`
