...
[XCMS] (import)  | `importFeatures(algorithm = "xcms", ...)`   | `xcmsSet` object              |
[XCMS3]          | `findFeatures(algorithm = "xcms3", ...)`    | centroid (`mzML` or `mzXML`)  | Uses `xcms::findChromPeaks()`
[XCMS3] (import) | `importFeatures(algorithm = "xcms3", ...)`  | `XCMSnExp` object             |
...
[KPIC2] (import) | `importFeatures(algorithm = "kpic2", ...)`  | Features from [KPIC2]         |
...
[XCMS] (import)  | `importFeatureGroups(type = "xcms", ...)`                | Imports an existing `xcmsSet` object.
[XCMS3]          | `groupFeatures(algorithm = "xcms3", ...)`                | Uses `xcms::groupChromPeaks()` and `xcms::adjustRtime()` functions
[XCMS3] (import) | `importFeatureGroups(type = "xcms3", ...)`               | Imports an existing `XCMSnExp` object.
[KPIC2]          | `groupFeatures(algorithm = "kpic2", ...)`                | Uses the [KPIC2] package
[KPIC2] (import) | `importFeatureGroups(type = "kpic2", ...)`               | Imports a `PIC set` object
[SIRIUS]         | `groupFeatures(anaInfo, algorithm = "sirius")`           | Finds _and_ groups features with [SIRIUS]
ProfileAnalysis  | `importFeatureGroups(type = "brukerpa", ...)`            | Import `.csv` file exported from Bruker ProfileAnalysis
TASQ             | `importFeatureGroups(type = "brukertasq", ...)`          | Imports a _Global result table_ (exported to Excel file and then saved as `.csv` file)
