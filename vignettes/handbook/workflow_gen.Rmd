...
The analysis information table can be manually constructed in `R` (e.g. through import of an `CSV` file), through a graphical interface with `newProject()` ([discussed later](#newProject)) or automatically by the `generateAnalysisInfo()` function. Here is an example of the latter for the example data in the `patRoonData` package:
