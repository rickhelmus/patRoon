...
IM collapse    | `convertMSFiles(algorithm = "im_collapse", ...)` | raw (Bruker TIMS) and ims (mzML) (uses [msdata](#fileTypes)) | centroid (`mzML` and `mzXML`) | Omits MS2 data by default.
...
> **_NOTE_** The IM collapse algorithm omits MS/MS data by default to save space and speed up file conversion. This algorithm is typically used in [post mobility assignment](#postIMS) IMS workflows, which do not use MS/MS data from centroided files. Set `includeMSMS=TRUE` to include MS/MS data.
...
* `minAbundanceAbs` / `minAbundanceRel`: Minimum absolute or relative abundance of a mass peak to be retained after averaging.
