...
Unfortunately, algorithms within the workflow may require different file types/formats, and it is often necessary to [convert raw data](#pre-treat) to one or more other file types/formats. In `patRoon` the choice of file type and format is primarily based on:
...
`"mzr"`         | uses [mzR] to read `centroid` files in the `mzML` and `mzXML` formats. This was always used in before `patRoon 3.0`.
...
IM collapse    | `convertMSFiles(algorithm = "im_collapse", ...)` | raw (Bruker TIMS) and ims (mzML) (uses [msdata](#fileTypes)) | centroid (`mzML` and `mzXML`)
...
The `piek` algorithm was introduced in `patRoon 3.0`, and extends the work done by @Dietrich2021. This algorithm finds features from extracted ion chromatograms (EICs) and can use different peak detection algorithms to find features. The EICs can be generated from one of the following:

* fixed-width _m/z_ bins
...
genEICParams <- getPiekGenEICParams(methodMZ = "bins", mzRange = c(50, 1000), mzStep = 0.01)
peakParams <- getDefPeakParams("chrom", "piek")
fList <- findFeatures(anaInfo, "piek", genEICParams = genEICParams, peakParams = peakParams)
...
genEICParams <- getPiekGenEICParams(methodMZ = "suspects")
...
genEICParams <- getPiekGenEICParams(methodMZ = "ms2", retRange = c(120, 600), minTIC = 1E5)
...
See `?getDefAvgPListParams` for all possible averaging parameters and `?generateMSPeakLists` for further details.
