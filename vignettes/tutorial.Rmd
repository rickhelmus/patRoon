...
options("patRoon.cache.mode" = "none")
```
...
Note that for larger project it may be preferred to export the `data.frame` to a _.csv_ file so it can be easily edited, for instance, with Excel. Alternatively, the `newProject()` function can be used to make this process even easier (briefly discussed [here](#newProject)).
