...
```{r plotEICs,fig.width=7}
plotEICs(fGroups, colourBy = "fGroups", showFGroupRect = FALSE, showPeakArea = TRUE,
         topMost = 1, showLegend = FALSE)
```

Note that here we only plot the most intense feature of a feature group here (as set by `topMost=1`). See the reference docs for many more parameters to these functions (e.g. `?plotEICs`).
...
plotEICs(components, "CMP6", fGroups)
```
