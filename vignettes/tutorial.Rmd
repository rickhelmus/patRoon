...
The last step of the workflow is typically to report all the data. The `reportHTML()` function combines all workflow data in an easy to use interactive `HTML` document.
...
reportHTML(fGroups, MSPeakLists = mslists, formulas = formulas)
```
...
reportHTML(fGroups, path = "../docs/examples", formulas = formulas, compounds = compounds,
           components = NULL, MSPeakLists = mslists, openReport = FALSE)
